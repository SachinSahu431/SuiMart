...
contract MarketPlace is Ownable,ERC721URIStorage {
    ...
    constructor(string memory _name, string memory _description) Ownable(msg.sender) ERC721("SuiMart", "AMART") {
        marketPlaceName = _name;
        marketPlaceDescription = _description;
    }

    function addProduct(string memory _name, string memory _description, uint _price, uint _quantity, string memory _ipfsLink) public onlyOwner {
    ...
    function editProduct(uint _id, string memory _name, string memory _description, uint _price, uint _quantity, string memory _ipfsLink) public onlyOwner {
        require(_id >= 0 && _id < products.length, "Invalid product id");
        require(bytes(_name).length > 0, "Name required");
        require(bytes(_description).length > 0, "Description required");
        require(_price > 0, "Price required");
